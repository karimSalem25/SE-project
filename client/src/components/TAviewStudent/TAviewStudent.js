...
const deleteStudent = (id) => {
    axios.delete('http://localhost:5000/viewS' + id).then(() => {
        window.location.reload(false); 
    })
}
    
  useEffect(() => {
    axios.get("http://localhost:5000/viewS").then((allStudents) => {
        TAsetStudentList(allStudents.data);
    });
  }, []);
    ...
    <>
      ...
      <TableContainer component={Paper}>
        <Table className={classes.table} aria-label="simple table">
          ...
          <TableBody>
            {TAstudentsList.map((student, key) => (
              <TableRow key={key}>
